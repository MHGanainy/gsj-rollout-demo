...
# Since library CP-71 (wheel 0.1.6 — this demo's floor) a corpus is three
# fields: name, owner, git:. The git host, the retrieval URL and the sandbox
# image are the ESTATE's — the estate tool answers them (here: bootstrap.py
# does, from config.yaml) — not the corpus's; a corpus is not bound to a
# runtime. Older corpora still carrying forgejo:/mcp:/sandbox_image validate
# with a deprecation warning, never a failure.
